...
def run_migrations_sync(db_path: Optional[str] = None):
    ...
    try:
        ...
        if not column_exists(conn, 'heuristics', 'project_path'):
            cursor.execute("ALTER TABLE heuristics ADD COLUMN project_path TEXT DEFAULT NULL")
        ...
        if not column_exists(conn, 'learnings', 'project_path'):
            cursor.execute("ALTER TABLE learnings ADD COLUMN project_path TEXT DEFAULT NULL")
        ...
        if not index_exists(conn, 'heuristic_project_path'):
            cursor.execute("CREATE INDEX heuristic_project_path ON heuristics (project_path)")
        ...
        if not index_exists(conn, 'learning_project_path'):
            cursor.execute("CREATE INDEX learning_project_path ON learnings (project_path)")
        ...
        try:
            ...
            if integrity_results and len(integrity_results) > 0:
                if integrity_results[0][0] != "ok":
                    logger.warning("  ! Database integrity issues detected, rebuilding indexes")
