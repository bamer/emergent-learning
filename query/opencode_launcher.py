    ...
    def ensure_server_running(self) -> bool:
        ...
        try:
            ...
            if resp.status_code == 200:
                print("[OpenCode] ✅ Server already running on port 4096")
                print("[OpenCode]    You can monitor it with: ps aux | grep opencode")
                return True
                ...
                for term in ['gnome-terminal', 'xterm', 'konsole', 'xfce4-terminal', 'alacritty']:
                    if subprocess.run(['which', term], capture_output=True).returncode == 0:
                        if term == 'gnome-terminal':
                            term_cmd = [term, '--', 'bash', '-c', 'opencode serve --port 4096; read -p "Press Enter to close..."']
                        elif term == 'xterm':
                            term_cmd = [term, '-e', 'bash', '-c', 'opencode serve --port 4096; read -p "Press Enter to close..."']
                        elif term == 'konsole':
                            term_cmd = [term, '-e', 'bash', '-c', 'opencode serve --port 4096; read -p "Press Enter to close..."']
                        else:
                            term_cmd = [term, '-e', 'bash', '-c', 'opencode serve --port 4096; read -p "Press Enter to close..."']
