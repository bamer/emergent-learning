...
class OpenCodeClient:
    ...
    def _call_server(
        self, prompt: str, timeout: int, agent: Optional[str] = None
    ) -> Optional[str]:
        ...
        try:
            ...
            if session_resp.status_code != 200:
                print(
                    f"Failed to create session: {session_resp.status_code}", flush=True
                )
                return None
            ...
            # 2. Send message
            # If agent specified, prepend @agent mention to prompt
            # This is the standard OpenCode way to invoke an agent
            message_text = prompt
            if agent:
                message_text = f"@{agent}\n\n{prompt}"
            ...
            # 4. Cleanup
            try:
                requests.delete(f"{self.server_url}/session/{session_id}", timeout=5)
            except:
                pass
