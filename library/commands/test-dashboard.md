...
- dev-browser server running: `~/.opencode/skills/dev-browser/server.sh`
- Dashboard backend: `cd ~/.opencode/emergent-learning/dashboard-app/backend && python -m uvicorn main:app --port 8888`
- Dashboard frontend: `cd ~/.opencode/emergent-learning/dashboard-app/frontend && bun run dev`
...
await page.screenshot({
  path: `~/.opencode/emergent-learning/memory/screenshots/dashboard-${timestamp}.png`,
  fullPage: true
});
...
# Kill and restart
pkill -f "dev-browser"
cd ~/.opencode/skills/dev-browser && bash server.sh &
```
