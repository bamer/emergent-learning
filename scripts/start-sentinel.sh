...
#   ./scripts/start-sentinel.sh --foreground
#
# Options:
...
if [ "$BACKGROUND" = true ]; then
    ...
    nohup python3 "$PROJECT_ROOT/Open_ELF/agents/dashboard_sentinel_complete.py" \
        --interval "$INTERVAL" \
        --log-level "$LOG_LEVEL" \
    ...
    echo ""
    python3 "$PROJECT_ROOT/Open_ELF/agents/dashboard_sentinel_complete.py" \
        --interval "$INTERVAL" \
        --log-level "$LOG_LEVEL"
fi