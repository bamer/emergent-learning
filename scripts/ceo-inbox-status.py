...
def print_summary(decisions: List[Dict]) -> None:
    ...
    critical = len(
        [
            d
            for d in decisions
            if d["priority"] == "critical" and d["status"] == "pending"
        ]
    )
    high = len(
        [d for d in decisions if d["priority"] == "high" and d["status"] == "pending"]
    )
...
def mark_decided(filename: str, decision: str) -> bool:
    ...
    success = update_file_frontmatter(
        path,
        {
            "status": "decided",
...
def main():
    ...
    parser.add_argument(
        "--status", choices=["pending", "decided", "deferred"], help="Filter by status"
    )
    parser.add_argument(
        "--priority",
    ...
    parser.add_argument(
        "--decide", type=str, metavar="FILE", help="Mark a file as decided"
    )
    parser.add_argument(
        "--decision", type=str, help="Decision text (use with --decide)"
    )
    ...
    decisions.sort(
        key=lambda d: (priority_order.get(d["priority"], 4), d.get("created", ""))
    )
