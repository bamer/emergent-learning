...
async def create_heuristic(heuristic: HeuristicCreate) -> ActionResult:
    ...
    with get_db() as conn:
        ...
        cursor.execute(
            ...
            INSERT INTO heuristics (
                rule, explanation, domain, confidence, times_validated, times_violated,
                is_golden, source_type, created_at, updated_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
            (
        ...
        return ActionResult(
            success=True,
            message=f"Created heuristic: {heuristic.rule[:50]}...",
            data={"heuristic_id": heuristic_id},
        )
