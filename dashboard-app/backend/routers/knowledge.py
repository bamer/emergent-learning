...
async def create_learning(learning: LearningCreate) -> ActionResult:
    ...
    with get_db() as conn:
        ...
        cursor.execute(
            ...
            INSERT INTO learnings (
                type, filepath, title, summary, tags, domain, severity, created_at, updated_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
            (
                learning.type,
        ...
        return ActionResult(
            success=True,
            message=f"Created learning: {learning.title}",
