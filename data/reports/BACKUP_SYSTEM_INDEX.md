...
|----------|---------|---------|
| `BACKUP_ROOT` | Backup location | `$HOME/.opencode/backups/emergent-learning` |
| `REMOTE_BACKUP_DEST` | Remote destination | `user@server:/backups` |
| `BACKUP_ENCRYPTION_KEY` | GPG key for encryption | `admin@example.com` |
| `FRAMEWORK_DIR` | Framework location | `$HOME/.opencode/emergent-learning` |
...
| Logs | Backup logs | `~/.opencode/backups/logs/` |
...
2. Check permissions: `ls -la ~/.opencode/backups/`
3. Check logs: `tail ~/.opencode/backups/logs/backup-daily.log`
