...
~/.opencode/emergent-learning/scripts/restore.sh list

# Restore latest backup
~/.opencode/emergent-learning/scripts/restore.sh latest

# Restore specific backup
~/.opencode/emergent-learning/scripts/restore.sh YYYYMMDD_HHMMSS

# Restore to git commit
~/.opencode/emergent-learning/scripts/restore-from-git.sh HEAD~5

# Verify backups (TODO: script needs to be created)
~/.opencode/emergent-learning/scripts/verify-backup.sh
```
...
0 2 * * * ~/.opencode/emergent-learning/scripts/backup.sh >> ~/.opencode/backups/backup.log 2>&1
...
1. **Check for safety backup:**
   ```bash
   ls -lah ~/.opencode/backups/emergent-learning/pre-restore-*.tar.gz
   ```
...
- `BACKUP_ROOT`: Backup destination (default: `~/.opencode/backups/emergent-learning`)
...
export BACKUP_ROOT=~/.opencode/backups/emergent-learning
...
# Test restore
~/.opencode/emergent-learning/scripts/restore.sh --force latest
...
if [ ! -f ~/.opencode/backups/emergent-learning/$(date +%Y%m%d)*.tar.gz ]; then
    echo "ERROR: No backup created today"
    # Send alert
fi

# Weekly verification (TODO: verify-backup.sh needs creation)
0 3 * * 0 ~/.opencode/emergent-learning/scripts/verify-backup.sh --alert-on-fail --email admin@example.com
```
