...
**Command**: `./scripts/verify-observability.sh` (TODO: script needs creation)
...
**File**: `~/.opencode/emergent-learning/logs/20251201.log`
...
**Search Command**:
```bash
grep "000192bb-14e4" ~/.opencode/emergent-learning/logs/20251201.log | wc -l
```
...
**Command**:
```sql
sqlite3 ~/.opencode/emergent-learning/memory/index.db "SELECT * FROM metrics ORDER BY timestamp DESC LIMIT 10;"
```
...
**Alert Files Created**:
```
~/.opencode/emergent-learning/alerts/
...
**CEO Escalation**:
```
~/.opencode/emergent-learning/ceo-inbox/alert_20251201_182914.md
```
...
**Command**: `./scripts/dashboard-simple.sh` (TODO: script needs creation)
...
```bash
grep -A5 "CORRELATION_ID.*log_get_correlation_id" ~/.opencode/emergent-learning/scripts/record-failure.sh
```
...
**Files Created**:
```bash
ls -lh ~/.opencode/emergent-learning/scripts/rotate-logs.sh
```
...
```bash
grep -E "(gzip|mtime.*90|log_dir_size)" ~/.opencode/emergent-learning/scripts/rotate-logs.sh
```
