...
- `LOG_DIR` - Directory for log files, default: ~/.opencode/emergent-learning/logs
...
- Location: `~/.opencode/emergent-learning/logs/`
...
```bash
# Check recent errors
grep ERROR ~/.opencode/emergent-learning/logs/$(date +%Y%m%d).log

# Analyze error patterns
sqlite3 ~/.opencode/emergent-learning/memory/index.db <<SQL
SELECT
...
```bash
# Find stale locks
find ~/.opencode/emergent-learning/.git -name "*.dir" -type d -mmin +30

# Remove stale locks (use with caution!)
find ~/.opencode/emergent-learning/.git -name "*.dir" -type d -mmin +30 -exec rmdir {} \;
```
...
- `AGENTS.md` - Configuration and usage instructions
