...
sqlite3 ~/.opencode/emergent-learning/memory/index.db "PRAGMA journal_mode"
...
```bash
# Restore from backup
cp ~/.opencode/emergent-learning/memory/index.db.backup_YYYYMMDD_HHMMSS \
...
```bash
# Watch for errors
tail -f ~/.opencode/emergent-learning/logs/$(date +%Y%m%d).log | grep ERROR

# Check database size
ls -lh ~/.opencode/emergent-learning/memory/index.db*

# Check freelist (should be < 100 after VACUUM)
sqlite3 ~/.opencode/emergent-learning/memory/index.db "PRAGMA freelist_count"
```
