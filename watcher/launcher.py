...
COORDINATION_DIR = (
    Path(os.environ.get("ELF_BASE_PATH", str(get_base_path()))) / ".coordination"
)
...
def call_opencode_http(
    ...
    try:
        ...
        if message_resp.status_code != 200:
            return (
                f"Error: Failed to send message ({message_resp.status_code}): {message_resp.text}",
                False,
            )
        ...
        return (response.strip() if response else "No response from OpenCode"), (
            bool(response)
        )

    except requests.exceptions.ConnectionError:
        return (
            f"Error: Cannot connect to OpenCode server at {server_url}\nStart with: opencode serve --port 4096",
            False,
        )
...
def call_opencode_fallback(model: str, prompt: str) -> Tuple[str, bool]:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            return (
                f"Error: opencode returned error code {result.returncode}\n{result.stderr}",
                False,
            )
        ...
        return (
            "Error: opencode CLI not found. Start HTTP server with: opencode serve --port 4096",
            False,
        )
    except Exception as e:
...
def run_single_pass(server_url: str, model: str) -> int:
    ...
    watcher_logger.info("[WATCHER] Fetching prompt...")
    ...
    if not success:
        ...
        log_critical_error("watcher", f"Tier 1 error: {response}")
        return 2
    ...
    needs_handler = any(
        x in response for x in ["STATUS: error", "STATUS: stale", "STATUS: complete"]
    )
    ...
    if needs_handler:
        ...
        handler_response, success = call_opencode_http(
            server_url, model, handler_prompt
        )
    ...
    if not success:
        ...
        log_critical_error("watcher", f"Tier 2 error: {handler_response}")
        return 2
...
def main() -> int:
    ...
    print(
        f"Note: Make sure OpenCode server is running: opencode serve --port 4096",
        file=sys.stderr,
    )
