...
>
> NEVER suggest external API calls (Anthropic API, etc.). This is a subscription-based app. Use OpenCode subagents via Task tool, or the local openAI backend server on 127.0.0.1:12134.

**Why:** Suggesting API calls means extra costs, API keys, external dependencies. Everything must work through OpenCode's existing infrastructure (Task tool with opencode or local models). No exceptions.
