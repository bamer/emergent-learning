...
try:
    ...
    _LOGGER = get_logger("model_detection")
except ImportError:
    import logging

    _LOGGER = logging.getLogger("model_detection")
    ...
    try:
        ...
        for flag in ["--version", "-v", "-V", "version"]:
            try:
                ...
                log_debug("model_detection", f"Timeout getting version for {cli_name}")
                continue
            except Exception as e:
                log_debug(
                    "model_detection", f"Error getting version for {cli_name}: {e}"
                )
                continue
        return "installed"  # Found but couldn't get version
    except Exception as e:
        log_debug("model_detection", f"CLI check failed for {cli_name}: {e}")
        return None
...
def load_routing_config(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    if config_path.exists():
        try:
            ...
            log_debug("model_detection", f"Failed to load routing config: {e}")
...
def format_models_for_context(models: Dict[str, Dict[str, Any]]) -> str:
    ...
    not_installed = [
        (name, info) for name, info in models.items() if not info.get("installed")
    ]
    ...
    if installed:
        for name, info in installed:
            ...
            if name == "opencode":
                lines.append(
                    f"- **opencode/big-pickle** (primary) [active] orchestrator"
                )
            else:
                lines.append(
                    f"- **{name}** v{version} [ready] {ctx_str} context | {strengths_str}"
                )
...
def suggest_model_for_task(
    ...
    if files:
        ...
        frontend_files = sum(
            1 for f in files if re.search(r"\.(tsx|jsx|vue|svelte|css|scss)$", f)
        )
...
if __name__ == "__main__":
    ...
    for task in test_tasks:
        ...
        print(f"\nTask: {task}")
        print(
            f"  Suggested: {suggestion['suggested']} ({suggestion['confidence']:.0%})"
        )
