...
void main() {
  ...
  // Base corona color
  vec3 finalColor = color * (1.0 + noise * 0.2);
  
  // Bright spots
  finalColor += vec3(1.0, 0.8, 0.4) * step(0.6, noise2) * 0.5;
  
  // Rim lighting
  float fresnel = pow(1.0 - dot(vNormal, vec3(0.0, 0.0, 1.0)), 2.0);
  finalColor += finalColor * fresnel;
...
void main() {
  ...
  // Atmosphere
  color += baseColor * fresnel * atmosphereDensity;
