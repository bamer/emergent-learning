#!/usr/bin/env python3
"""
FIXED VERSION: Learning Extractor - Processes session JSONL logs to extract learnings.

Changes:
1. Added project_path detection and recording
2. Fixed domain validation (rejects invalid domains)
3. Proper sanitization of domain field
4. Added [LEARNED] marker without domain support (defaults to general)
"""
...
def validate_domain(domain: str) -> str | None:
    ...
    if not VALID_DOMAIN_PATTERN.match(domain):
        print(f"DEBUG: Invalid domain rejected: {domain}", file=sys.stderr)
        return None

    # Domain must be reasonable (1-30 chars)
    if len(domain) < 2 or len(domain) > 30:
        print(
            f"DEBUG: Domain length rejected: {domain} (length: {len(domain)})",
            file=sys.stderr,
        )
        return None
...
def extract_learned_markers(text: str) -> List[Dict[str, str]]:
    """Extract [LEARNED:domain] markers from text with strict validation."""
    # Pattern 1: [LEARNED:domain] lesson content
    pattern1 = r"\[LEARNED:\s*([a-z0-9\-]+)\]\s*(.+?)(?=\[LEARNED|$)"
    # Pattern 2: [LEARNED] lesson content (no domain = general)
    ...
    for match in re.finditer(pattern1, text, re.IGNORECASE | re.DOTALL):
        ...
        domain = validate_domain(domain_str)
        if not domain:
            print(f"Skipping [LEARNED:{domain_str}] - invalid domain", file=sys.stderr)
            continue
    ...
    # Extract without domain (default to general)
...
def store_learning_in_db(domain: str, lesson: str, source: str = "extractor"):
    """Store a learning directly in the database with project_path."""
    try:
        # Validate domain before storing
        validated_domain = validate_domain(domain)
        if not validated_domain:
            print(f"Skipping learning with invalid domain: {domain}", file=sys.stderr)
            return False
        ...
        conn.execute(
            ...
            (type, domain, description, outcome, timestamp, context, project_path)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """,
            (
                "observation",
    ...
    for entry in all_entries:
        ...
        for field in ["input_summary"]:
            ...
            for marker in markers:
                if store_learning_in_db(marker["domain"], marker["lesson"]):
                    results["learnings"] += 1
