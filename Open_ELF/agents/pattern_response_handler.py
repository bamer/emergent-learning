...
class PatternResponseHandler:
    ...
    def _record_pattern_as_learning(
        ...
        try:
            ...
            # Record as learning (proper pipeline stage)
            cursor.execute(
                """
                INSERT INTO learnings
                (type, title, summary, domain, severity, created_at)
                VALUES (?, ?, ?, ?, ?, ?)
            """,
                (
                    "observation",  # type
                    pattern,  # title
                    "Pattern detected by Sentinel monitoring and auto-recorded",  # summary
            ...
            logger.info(f"✅ Pattern recorded as learning: {pattern}")
            return True
    ...
    def _record_to_chronicle(
        self, pattern: str, context: Dict[str, Any], recommendations: List[str]
    ) -> bool:
...
def main():
    ...
    for pattern in test_patterns:
        ...
        result = handler.handle_pattern(
            pattern, {"metrics": {"activity_score": 1}, "timestamp": datetime.now()}
        )
