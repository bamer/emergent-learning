...
Retrieves user prompts from previous Opencode sessions.

### Basic Usage

```bash
# Last 5 prompts from most recent session
python ~/.opencode/emergent-learning/sessions/get_last_prompts.py

# Last 10 prompts
python ~/.opencode/emergent-learning/sessions/get_last_prompts.py --limit 10

# Search across multiple recent sessions
python ~/.opencode/emergent-learning/sessions/get_last_prompts.py --all-sessions

# JSON output for scripting
python ~/.opencode/emergent-learning/sessions/get_last_prompts.py --json
```
...
python ~/.opencode/emergent-learning/sessions/search.py "auth bug we fixed"

# Search recent history
python ~/.opencode/emergent-learning/sessions/search.py "what failed yesterday" --days 1

# Limit results
python ~/.opencode/emergent-learning/sessions/search.py "database migrations" --limit 5

# JSON output
python ~/.opencode/emergent-learning/sessions/search.py "grep searches" --json
```
...
1. **Opencode history files** (`~/.opencode/projects/*/[session-id].jsonl`)
   - Contains full conversation: user prompts and assistant responses
   - Used by `get_last_prompts.py`

2. **Framework session logs** (`~/.opencode/emergent-learning/sessions/logs/`)
