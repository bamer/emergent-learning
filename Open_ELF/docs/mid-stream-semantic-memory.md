...
4. Embeds using Ollama (`nomic-embed-text`, 768 dimensions)
...
```bash
# Install Ollama for semantic embeddings (recommended)
# See: https://ollama.com/download
ollama pull nomic-embed-text

# Without it, falls back to OpenAI (if API key) or keyword matching
```
...
Enable thinking blocks in Opencode so the hook has content to extract:
...
CREATE TABLE heuristic_embeddings (
    ...
    model_version TEXT,       -- 'nomic-embed-text'
...
```json
{
  ...
  "pending_plan_validations": [
    {
      "plan_file": "~/.opencode/plans/auth.md",
      "heuristic_ids": [5, 12],
...
```bash
# Verify thinking is enabled in Opencode
cat ~/.opencode/settings.json | grep -A5 thinking

# Check transcript format
head -5 ~/.opencode/projects/*/session.jsonl
```
...
ls -la ~/.opencode/emergent-learning/.embedding_cache/
...
### 0.6.3 (2026-01-29)
- 🔄 Migrated from sentence-transformers to Ollama (nomic-embed-text)
- ✨ Initial release
