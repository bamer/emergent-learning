...
class OptimizedOpenCodeClient:
        ...
        with self.session_lock:
            ...
            if not self.opencode_session_id:
                ...
                try:
                    response = self.http_session.post(
                        ...
                        json={
                            "title": f"ELF Persistent Session - {datetime.now().strftime('%Y-%m-%d %H:%M')}"
                        },
                    ...
                    if response.status_code in [200, 201]:
                        ...
                        logger.error(
                            f"❌ Failed to create session: {response.status_code} - {response.text[:200]}"
                        )
                        return False
            ...
            try:
                response = self.http_session.get(
                    f"{self.base_url}/session/{self.opencode_session_id}", timeout=10
                )
                    ...
                    with self.session_lock:
                        ...
                        try:
                            ...
                            response = self.http_session.post(
                                ...
                                json={
                                    "parts": [
                                        {
                                            "type": "text",
                                            "text": "[HEARTBEAT] ELF session keepalive",
                                        }
                                    ]
                                },
                                timeout=30,
                            ...
                            if response.status_code in [200, 201, 204]:
                                ...
                                logger.warning(
                                    f"⚠️ Heartbeat failed: {response.status_code}"
                                )
                                self.opencode_session_id = None
                        except Exception as e:
    ...
    def send_message(
        ...
        try:
            ...
            if response.status_code in [200, 201, 204]:
                ...
                logger.error(
                    f"❌ Message send failed: {response.status_code} - {response.text[:200]}"
                )
    ...
    def health_check(self) -> bool:
        ...
        try:
            response = self.http_session.get(f"{self.base_url}/", timeout=10)
...
if __name__ == "__main__":
    ...
    if client.health_check():
        ...
        success, response = client.send_message(
            "Hello, this is a test message from optimized client"
        )
        if success:
