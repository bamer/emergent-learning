...
class MissionEngine:
    ...
    def _setup_logging(self):
        ...
        if self.config.log_to_file:
            log_file = (
                self.config.logs_dir / f"mission_engine_{datetime.now():%Y%m%d}.log"
            )
    ...
    def execute_mission(self, mission_id: str) -> MissionResult:
        ...
        if not mission:
            return MissionResult(
                success=False, output=f"Mission non trouvée: {mission_id}"
            )
        ...
        try:
            ...
            is_critical = self._is_critical_mission(
                mission, analysis_result.get("analysis")
            )
            ...
            if is_critical and self.config.auto_escalate_critical:
                ...
                ceo_result = self._escalate_to_ceo(
                    mission, analysis_result.get("analysis")
                )
            ...
            actions = self._extract_actions(
                ceo_decision or analysis_result.get("analysis", "")
            )
    ...
    def _execute_agent_analysis(self, mission: Mission) -> Dict[str, Any]:
        ...
        if not self.agent_manager:
            return {"success": False, "error": "AgentManager non disponible"}
...
5. Métriques de succès"""
    ...
    def _escalate_to_ceo(self, mission: Mission, analysis: str) -> Dict[str, Any]:
        ...
        """
        if not self.agent_manager:
            return {"success": False, "error": "AgentManager non disponible pour CEO"}
