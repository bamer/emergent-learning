...
class Watcher:
    ...
    def collect_metrics(self) -> Dict[str, Any]:
        ...
        metrics["services"] = {
            ...
            "learning_capture": self.check_process_running("background-learning-capture.py"),
        }
    ...
    def detect_patterns(self, metrics: Dict[str, Any]) -> List[str]:
        ...
        if all(recent_activity[i] <= recent_activity[i-1] 
        ...
        if len(self.pattern_memory) >= 10:
            old_learnings = self.pattern_memory[-10]["metrics"]["database"].get("learnings", 0)
    ...
    def analyze_with_ai(self, metrics: Dict[str, Any], patterns: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.get("success"):
                ...
                return self._fallback_analysis(
                    metrics, 
                    result.get("error", "AI analysis failed")
                )
    ...
    def _extract_priority_actions(self, response: str) -> List[str]:
        ...
        for line in lines:
            lower_line = line.lower()
            if any(word in lower_line for word in ["priority", "immediate", "urgent", "action"]):
                in_priority = True
    ...
    def _fallback_analysis(self, metrics: Dict[str, Any], error_msg: str) -> Dict[str, Any]:
    ...
    def create_escalation(self, analysis: Dict[str, Any], metrics: Dict[str, Any]) -> Optional[str]:
        """Create escalation file in CEO inbox if critical."""
        ...
        try:
            ...
            escalation_file = CEO_INBOX_DIR / f"{escalation_id}.md"
            
            with open(escalation_file, "w") as f:
                f.write(f"# Critical Escalation: {escalation_id}\n\n")
                ...
                f.write("---\n\n")
                f.write("This escalation was automatically generated by the Watcher agent.\n")
            ...
            if LOGGING_AVAILABLE:
                log_watcher_check(
                    tier=1,
                    status="critical",
                    summary=f"Critical escalation created: {escalation_id}",
    ...
    def log_cycle(self, metrics: Dict[str, Any], analysis: Dict[str, Any], 
                  patterns: List[str], escalation_file: Optional[str]):
    ...
    def display_status(self, metrics: Dict[str, Any], analysis: Dict[str, Any],
        ...
        print(f"\n{'='*70}")
        print(f"🔍 Watcher v3.0 - Level 1 Agent - {datetime.now().strftime('%H:%M:%S')}")
        print(f"{'='*70}")
        print(f"{status_emoji} Status: {analysis.get('status', 'unknown').upper()}")
        print(f"🤖 AI Analysis: {'Yes' if analysis.get('ai_processed') else 'No (fallback)'}")
        ...
        print(f"  Heuristics: {db.get('heuristics', 0)} ({db.get('golden_rules', 0)} golden)")
        print(f"  Trails: {db.get('trails', 0)} | Pheromone: {db.get('pheromone_trails', 0)}")
    ...
    def run_cycle(self) -> Dict[str, Any]:
        ...
        if should_run_ai and self.agent_manager:
            ...
            analysis = self._fallback_analysis(
                metrics, 
                "AI analysis skipped (interval not reached)" if not should_run_ai else "AI unavailable"
            )
    ...
    def run_continuous(self):
        ...
        logger.info(f"   AgentManager: {'✅ Available' if self.agent_manager else '❌ Unavailable'}")
