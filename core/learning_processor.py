...
class LearningProcessor:
    ...
    def _detect_domains(self, tool_input: Dict, tool_name: str) -> List[str]:
        ...
        domain_keywords = {
            "security": ["security", "vulnerability", "encrypt", "password", "auth", "permission", "validate"],
            "performance": ["performance", "speed", "cache", "optimize", "memory", "cpu", "async"],
        ...
        for pattern, reason in failure_patterns:
            if re.search(pattern, content, re.MULTILINE):
                ...
                if match:
                    ...
                    is_false_positive = any(re.search(fp, context) for fp in false_positives)
    ...
    def _validate_heuristics(self, heuristic_ids: List[int], outcome: str) -> Tuple[int, int]:
    ...
    def _record_workflow_trails(self, event: ToolEvent, outcome: str) -> int:
        ...
        try:
            ...
            strength = 1.0 if outcome == "success" else 0.5 if outcome == "unknown" else 0.3
    ...
    def _determine_scent(self, event: ToolEvent, outcome: str) -> Optional[str]:
        ...
        if tool_lower in ("glob", "grep") or "discover" in str(event.tool_input).lower():
            return "discovery"
    ...
    def _extract_implicit_learnings(self, content: str) -> List[Dict]:
        ...
        heuristic_indicators = [
            "should", "always", "never", "must", "don't", "avoid",
            "prefer", "recommend", "best practice", "rule of thumb",
            "lesson", "insight", "key takeaway", "critical to", "important to"
        ]
    ...
    def _extract_explicit_learnings(self, content: str) -> List[Dict]:
        ...
        for domain_hint, learning_text in matches:
            ...
            if learning_text:
                learnings.append({
                    "rule": learning_text,
                    "domain": domain_hint.strip() if domain_hint.strip() else "general",
                    "confidence": 0.8,
                    "source": "explicit"
                })
    ...
    def _check_golden_rule_promotion(self):
        ...
        try:
            ...
            for candidate in candidates:
                ...
                print(f"[LEARNING] Promoted to golden rule: {candidate['rule'][:50]}...", file=sys.stderr)
    ...
    if args.decay_trails:
        ...
        if spots:
            ...
            for spot in spots:
                ...
                print(f"    Trails: {spot['trail_count']}, Strength: {spot['total_strength']:.2f}")
