...
class MonitoringAPI:
    ...
    def _get_learning_stats(self, conn: sqlite3.Connection) -> Dict[str, int]:
        ...
        try:
            ...
            cursor.execute(
                "SELECT COUNT(*) as count FROM heuristics WHERE is_golden = 1"
            )
    ...
    def _get_tool_detection_stats(self, conn: sqlite3.Connection) -> Dict[str, int]:
        ...
        try:
            ...
            result = cursor.fetchone()
            return {
                "tools_detected": result["total"] if result and result["total"] else 0
            }
        except:
    ...
    def _get_last_activity(self, conn: sqlite3.Connection) -> Optional[str]:
        ...
        try:
            ...
            result = cursor.fetchone()
            return (
                result["last_activity"] if result and result["last_activity"] else None
            )
        except:
            return None
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self.stats_lock:
            ...
            if conn:
                try:
                    # Events per minute
                    stats["events"]["per_minute"] = self._calculate_events_per_minute(
                        conn
                    )
...
class MonitoringHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/":
            ...
            stats = self.monitoring_api.get_stats()
            self.wfile.write(
                json.dumps(stats.get("architecture", {}), indent=2).encode()
            )
